...
def selectSeries(series_numbers, SeriesDescription, CountOfImages):
    ...
    window_select_series['-SERIES LIST-'].update(chance, set_to_index=index_of_Series, scroll_to_index=0)

    while True:
        event, values = window_select_series.read()

        if event in ('MouseWheel:Down', 'Down:40', 'Next:34') and index_of_Series < len(chance) - 1:
            index_of_Series += 1
            window_select_series['-SERIES LIST-'].update(set_to_index=index_of_Series, scroll_to_index=index_of_Series)

        elif event in ('MouseWheel:Up', 'Up:38', 'Prior:33') and index_of_Series > 0:
            index_of_Series -= 1
            window_select_series['-SERIES LIST-'].update(set_to_index=index_of_Series, scroll_to_index=index_of_Series)

        elif event in (sg.WIN_CLOSED, 'OK', 'Escape:27'):
            selected_Series_Number = series_numbers[index_of_Series]
            window_select_series.close()
            break

        window_select_series['-SERIES LIST-'].update(chance, set_to_index=index_of_Series,
                                                     scroll_to_index=index_of_Series)
...
def readDICOMDIR(dicom_dir_path):
    ...
    selected_Series_Number = selectSeries(SeriesNumbers, SeriesDescription, CountOfImages)
...
def readDicomFiles(dicom_path):
    ...
    selected_Series = selectSeries(series_numbers, SeriesDescription, CountOfImages)
    ...
    ss = [ss for ss in slices if ss.SeriesNumber == selected_Series]
    for f in ss:
        if hasattr(f, 'SliceLocation'):
            new_slices.append(f)
        else:
...
def read_dicom_set(dicom_path):
    ...
    path = dicom_path + '/'
    dicom_dir_path = (path + 'DICOMDIR') if os.path.isfile(path + 'DICOMDIR') else None
    if dicom_dir_path is not None:
        # print(dicom_dir_path)
        slices = readDICOMDIR(dicom_dir_path)
    else:
    ...
    frame_Size = (slices[0].Rows, int(slices[0].Rows * (Height_image / Length_image)))
    return slices, frame_Size
...
def get_images_from_slice(data_array, images_path, slices, step=1):
    ...
    with open(dir_info, 'w') as f:
        ...
        f.write(f'Размер изображения: {frame_Size} \n')
        ...
        for y in range(start_pos, stop_pos):
            ...
            save_slice_to_image(out_image_name, coronal_slice, slices[0].WindowCenter,
                                slices[0].WindowWidth, frame_Size)
            count += 1
...
def readDicomFolder(dicom_path, images_path):
    ...
    slices, frame_Size = read_dicom_set(dicom_path)
