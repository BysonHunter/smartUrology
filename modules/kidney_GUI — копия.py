...
def view_detected_images(input_path, frameSize=(512, 512)):
    ...
    file_list_column = [
        ...
        [sg.Text(list_text[1], justification='center', size=(45, 2))],
        [sg.HSeparator()],
        [sg.Button(button_text=buttons[19])],
        [sg.Cancel(button_text=buttons[5], size=(22, 2), button_color='teal')],
    ]
...
def main():
    ...
    while True:
        ...
        if event in (buttons[0], 'Open'):
            ...
            if input_path != '' and os.path.exists(input_path):
                png_files = [os.path.join(input_path, f) for f in os.listdir(input_path) if f.lower().endswith('.png')]
            if not png_files or input_path == '':
